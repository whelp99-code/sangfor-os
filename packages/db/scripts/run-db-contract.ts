...
function listMigrationsThroughU020(): string[] {
  return readdirSync(join(REAL_PRISMA_DIR, 'migrations'), { withFileTypes: true })
    .filter((d) => d.isDirectory())
    .map((d) => d.name)
    .filter((name) => name !== NEW_MIGRATION_NAME_U021 && name !== NEW_MIGRATION_NAME_U024 && name !== NEW_MIGRATION_NAME_U032 && name !== NEW_MIGRATION_NAME_U033 && name !== NEW_MIGRATION_NAME_U034 && name !== NEW_MIGRATION_NAME_U035 && name !== NEW_MIGRATION_NAME_U036 && name !== NEW_MIGRATION_NAME_U037)
    .sort();
}
  ...
  try {
    await withIsolatedPostgres(
      ...
      async (ctx: any) => {
        ...
        // (71 as of U037 — RoleChangeRequest's U012 reclassification plus every later CHILD_VIA_FK
        // registration, most recently U036's DemoLicense [68 -> 69] and U037's DeliveryAcceptance +
        // RenewalReminderEvent [69 -> 71]) must be
        // updated by any future unit that adds a new CHILD_VIA_FK model, exactly as U017/U018/U019
        // updated it here.
        if (scopeCheckJson.tallies.CHILD_VIA_FK !== 71 || scopeCheckJson.tallies.GLOBAL_SHARED !== 13) {
  ...
  try {
    await withIsolatedPostgres(
      ...
      async (ctx: any) => {
        ...
        verifyViewIntegrity(view, [...throughU020, NEW_MIGRATION_NAME_U021, NEW_MIGRATION_NAME_U024, NEW_MIGRATION_NAME_U032, NEW_MIGRATION_NAME_U033, NEW_MIGRATION_NAME_U034, NEW_MIGRATION_NAME_U035, NEW_MIGRATION_NAME_U036, NEW_MIGRATION_NAME_U037]);
...
async function runRoleChangeSuite(evidenceDir: string): Promise<number> {
  ...
  try {
    ...
    await withIsolatedPostgres(
      ...
      async (ctx: any) => {
        const conn = parseConn(ctx.databaseUrl);
        const before = readdirSync(join(REAL_PRISMA_DIR, 'migrations'), { withFileTypes: true }).filter((d) => d.isDirectory()).map((d) => d.name).filter((name) => name !== NEW_MIGRATION_NAME_U024 && name !== NEW_MIGRATION_NAME_U032 && name !== NEW_MIGRATION_NAME_U033 && name !== NEW_MIGRATION_NAME_U034 && name !== NEW_MIGRATION_NAME_U035 && name !== NEW_MIGRATION_NAME_U036 && name !== NEW_MIGRATION_NAME_U037).sort();
        ...
        try {
          ...
          addMigrationToView(view, NEW_MIGRATION_NAME_U036);
          addMigrationToView(view, NEW_MIGRATION_NAME_U037);
          verifyViewIntegrity(view, [...before, NEW_MIGRATION_NAME_U024, NEW_MIGRATION_NAME_U032, NEW_MIGRATION_NAME_U033, NEW_MIGRATION_NAME_U034, NEW_MIGRATION_NAME_U035, NEW_MIGRATION_NAME_U036, NEW_MIGRATION_NAME_U037]);
