...
export interface GenerateThreadsResult {
  threadsCreated: number;
  totalMessages: number;
  groupingMethod: string;
}

interface MailMessageLike {
  id: string;
  subject: string;
  fromEmail: string;
  createdAt: Date;
 ...
 */
function groupMessagesIntoThreads(messages: MailMessageLike[]) {
  const threads = new Map<string, MailMessageLike[]>();
...
/**
 * Group recent mail messages into insight threads and persist them.
 * Extracted from apps/web route to decouple presentation from persistence.
 */
export async function generateMailInsightThreads(
  limit: number,
): Promise<GenerateThreadsResult> {
  // 메일 메시지 조회
  ...
  for (const [threadKey, threadMessages] of threadMap) {
    ...
    const participantDomains = [
      ...new Set(threadMessages.map((m) => m.fromEmail.split("@")[1])),
    ];
