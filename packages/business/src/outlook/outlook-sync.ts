...
import { getDelegatedConnection, syncDelegatedOutlook } from './outlook-graph'
...
export async function syncOutlook(options: SyncOutlookOptions = {}): Promise<SyncResult> {
  const preferDelegated = options.preferDelegated ?? true
  if (preferDelegated) {
    ...
    if (connection.connected) {
      try {
        ...
        return { success: false, mode: 'delegated', error: 'sync_failed' }
      }
    }
  }
