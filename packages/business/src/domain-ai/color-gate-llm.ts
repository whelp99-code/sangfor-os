...
- orange(가치): 고객 페인 부합·차별점·마진/성공지표
- gray(근거): 내용 완결성·구체성·논리
...
export async function verifyProposalColorGate(
  ...
  const system = `당신은 한국 B2B 제안서 품질 검증 AI입니다. 아래 5개 렌즈로 제안서를 냉정하게 평가하세요.
${LENS_DEF}
각 렌즈마다 pass(boolean)와 note(한국어 한 문장 근거)를 판정하고, 전체 pass(모든 렌즈가 합격 수준인지)와 summary(한 문장)를 매기세요.
