...
node -e 'const fs=require("fs"); const [path,candidate,deployment,project,api,web,backup,domain]=process.argv.slice(1); fs.writeFileSync(path, JSON.stringify({schemaVersion:1,candidateSha:candidate,deploymentId:deployment,projectName:project,images:{api:`${api}:${candidate}`,web:`${web}:${candidate}`},backup:`${backup}/predeploy-${deployment}.dump`,domain,deployedAt:new Date().toISOString()},null,2)+"\n",{mode:0o600})' \
  ".local-prod/deployments/${DEPLOYMENT_ID}.json" "$EXPECTED_SHA" "$DEPLOYMENT_ID" "$PROJECT_NAME" "$API_IMAGE" "$WEB_IMAGE" "$BACKUP_DIR" "$APP_DOMAIN"
