...
function validEnvironment() {
  return {
    APP_DOMAIN: "sangfor.internal.test", BACKUP_DIR: "/var/backups/sangfor-os", POSTGRES_PASSWORD: "p".repeat(32), SANGFOR_APP_DB_PASSWORD: "a".repeat(32), SANGFOR_RUNTIME_DB_PASSWORD: "t".repeat(32), REDIS_PASSWORD: "r".repeat(32),
...
describe("production deploy verifier", () => {
  ...
  it("accepts complete, distinct production credentials", () => {
    assert.deepEqual(validateProductionEnvironment(validEnvironment()), { ok: true, requiredCount: 23 });
  });
  ...
  if (process.env.RUN_DOCKER_COMPOSE_CONTRACT === "1") {
    it("renders and validates the real production Compose model", () => {
      ...
      try {
        assert.equal(verifyProductionDeploy(envFile).serviceCount, 8);
      } finally {
