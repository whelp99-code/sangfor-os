...
IFS=$'\t' read -r TARGET_SHA API_TAG API_ID WEB_TAG WEB_ID DEPLOYMENT_COMPOSE < <(node scripts/production-deployment-receipt.mjs verify --receipt "$RECEIPT" --project "$PROJECT_NAME" --api-image "$API_IMAGE" --web-image "$WEB_IMAGE" --deployment-dir "$DEPLOYMENT_DIR")
