...
> **최초 작성**: 2026-06-29 (2026-06-28 작업 일괄 정리) · **마지막 갱신**: 2026-08-15

> **Work tracker (mandatory)**: [TRACKER.md](TRACKER.md) — GitHub Issues/labels primary (not Linear). JM=code, Actions=verify, BLRO=runtime (`ssh blro`).
...
## 변경 이력
- **2026-08-15 (all Orca projects tracker)**: Orca 등록 프로젝트 전체에 GitHub Issues 트래커 팩 적용(sangfor-os, engineer-mcp, jarvis-ai, Toss-finance, blro-os-next, gitgauge, dev-plan-tool + local jm-loop-miner/jmai-os-pack). Linear 탭 참고 전용. `scripts/apply-tracker-standard-to-repo.sh`.
- **2026-08-15 (Orca UI GitHub-first)**: Orca Linear 탭/Workspace board는 참고·세션용. 일감 정본은 GitHub Issues. `scripts/tracker.sh`로 list/open/blro 필터. docs/TRACKER.md에 UI 가이드 추가.
- **2026-08-15 (GitHub Issues tracker standard)**: Linear를 primary tracker로 쓰지 않는다. 계획·진행·완료의 단일 작업 단위는 GitHub Issue + labels(`status:*`, `ready-for-blro`, `ops`, `blro`, `agent`, `ci-blocked`)이다. PR 템플릿/이슈 템플릿/docs/TRACKER.md/AGENTS.md 진입점을 추가했다. GitHub Projects v2는 `gh` 토큰에 `project` 스코프가 있을 때 선택적으로 쓰며, 스코프 없을 때 labels가 정본이다. 런타임 적용은 BLRO(`ssh blro`, `/home/blro/sangfor-os`)에서만 한다.
- **2026-08-15 (Phase 2/3 종료 — PR #198 + 로컬 운영 검증)**: `feat/mail-candidate-triage`를 PR #198로 main에 머지했다(CI 10/10 green: secrets-scan/static-checks/release-gate-unit/test/build/e2e + service jobs). 로컬에 Docker/sudo가 없어 user-space PostgreSQL 16.14를 :5434에 기동한 뒤 `pnpm db:migrate:deploy`로 76개 migration(포함 `20260812190000_unify_autopilot_policy_key`)을 fresh DB에 적용하고 `pnpm db:seed`로 demo-project를 심었다. seed 결과 `autonomy_policies`는 domain 6개 전부 `decisionType=autopilot_approve`/`mode=observe`이며 legacy `mail_candidate_approve`는 0건. ground-truth dry-run CLI는 `writesPerformed:0`; `importMailGroundTruth`로 승인 대장 6건을 demo-project `policy_memories(project_entity_role)`에 적재했고, `revertMailGroundTruthImport` dry+live(matched/reverted=6) 후 재import로 active 6건 상태를 복구했다. 프로덕션 배포 타깃은 이 저장소에 아직 없다(`cd.yml` build-check only). 비밀번호 최소길이 16→8 완화는 스코프 밖 보안 후퇴로 커밋하지 않았다.
