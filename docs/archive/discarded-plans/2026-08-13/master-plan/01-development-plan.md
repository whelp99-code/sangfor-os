...
1. WP-A~E의 PR 5개가 main에 머지되고 각 Acceptance가 증거 파일로 남아 있다.
   ✅ **충족(2026-07-07 최종검증 — 판정 승격)** — PR 5개(#96~#98, #100, #101) 전부 main 머지 완료(WP-E #101: 2026-07-07T10:44:29Z, `22de4b5`). 각 Acceptance 증거 파일도 전부 `.agents/results/`에 존재: WP-A(`wp-a-gates.md`+`wp-a3-live.md`), WP-B(`wp-b-consistency.md`), WP-C(`wp-c-gates.md`+`wp-c3-pipeline.md`), WP-D(`wp-d-live.md`), **WP-E(`wp-e-ia.md`, 신설 — 이전 라운드의 "전용 증거 파일 없음" 갭 해소)**. e2e를 실차단화한 #99도 같은 웨이브.
2. `pnpm lint && typecheck && test && build` + `CI_INTEGRATION=1` 통합 + `pnpm test:e2e` 전부 green.
   ✅ **충족(2026-07-07 최종검증 재확인)** — 6개 명령 전부 exit 0: lint(0 errors)/typecheck/test(api 63+web 134 passed)/build/`CI_INTEGRATION=1`(68 files·535 passed)/`pnpm test:e2e`(67 passed/34 skipped, playwright `webServer`가 스스로 서버를 부팅해 실행됨을 로그로 확인). 증거: `.agents/results/2026-07-07-final-verification.md` §①(개별 WP 게이트 증거는 `wp-a-gates.md`/`wp-c-gates.md`).
3. 진행중 딜 수치가 전 화면에서 일치한다 (02 검증서 SQL 체크 C-1).
   ✅ **충족(2026-07-07 최종검증 — 현재값 갱신)** — 5표면(홈 KPI/executive API/딜 칸반/opportunities redirect/daily-report) 전부 **64** = DB 기준값 64(LEAD 38+QUALIFIED 4+PROPOSAL 16+POC 0+NEGOTIATION 6). WP-B 검증 시점(56)에서 파이프라인이 계속 가동되며 자연 증가한 현재값 기준으로도 5표면 정합이 그대로 유지됨을 재확인. 증거: `.agents/results/2026-07-07-final-verification.md` §② + 스크린샷(`final-verify-screens/02-*.png`).
4. 메일 후보 적체 0, 재무-engagement 연결 리포트 존재, `"demo-project"` 잔존 0(시드 제외).
   🔶 **부분충족(2026-07-07 최종검증 갱신)** — **재무-engagement 연결 리포트**: ✅ 존재(변동 없음). **`"demo-project"`/`MOCK_PROJECTS` 잔존**: ✅ **apps/web 0건**으로 재확인(`grep -rn '"demo-project"' apps/web/src` → 0, `grep -rn "MOCK_PROJECTS" apps/web/src` → 0) — `packages/business` 잔존 **86건**(이전 라운드 추정 76건에서 실측 갱신)은 계획대로 06 문서(4차 고도화)로 이월 확정. **메일 후보 적체**: 🔶 여전히 0 아님 — `proposed` **1,098건**(이전 라운드 1,074건에서 소폭 증가; `converted` 171건 누적으로 파이프라인 가동 자체는 증명됨). 적체 해소는 05 문서(분류기 신뢰도 캘리브레이션) 선결 과제로 이월 유지. **종합**: 3개 하위조건 중 2개(재무 리포트·demo-project) 충족, 1개(메일 적체)는 미충족이라 항목 전체는 부분충족 유지. 증거: `.agents/results/2026-07-07-final-verification.md` §③⑤.
5. "준비 중" 마커 0건. 사이드바의 모든 메뉴가 실화면.
   ✅ **충족(2026-07-07 최종검증 — 판정 승격)** — `grep -rn "준비 중\|준비중" apps/web/src` → 2건, 전부 `win-work-panel.tsx`(120행 "준비중", 215행 "준비중 (데이터 연결 전)")의 실제 도메인 상태 라벨(계약상태 배지류)이며 UI 플레이스홀더/coming-soon 스텁이 아님을 최종검증에서 재확인 — 이 조건의 취지(가짜 기능 은폐 방지) 위반이 아닌 허용 목록으로 판정해 부분충족에서 충족으로 승격. 증거: `.agents/results/2026-07-07-final-verification.md` §④.
6. 프로젝트 허브에서 제안 생성→5렌즈 verdict→사람 승인→문서 승격→자율도 갱신 루프가 라이브로 재연된다.
   🔶 **부분충족(2026-07-07 최종검증 갱신 — 신규 결함 발견)** — **A. 제안 승인→문서 승격**: ✅ 라이브 재연(인카금융서비스 프로젝트, pending 제안 승인→brass 링크→`/proposals/[id]` 문서 렌더, `domain_decision_logs` outcome=approved). **C. 프로젝트 허브 손익 실값**(재무-engagement 연결의 최종 라이브 확인): ✅ 롯데건설 리뉴얼-2대 딜 — 매출 3,740,000원/마진 2,541,000원(67.9%), 매입 0원은 해당 engagement에 연결할 매입 세금계산서 자체가 DB에 0건이라 정직한 값(연결 결함 아님). **B. AI 커맨드바 토스트 가시성 — 미충족, 신규 결함**: 토스트 DOM/상태는 정상(텍스트 발화, `opacity 1`, z-50)이지만, 토스트가 커맨드바 `Card` 바깥 위쪽(`absolute bottom-full`)에 렌더되는데 shadcn `Card` 베이스 클래스의 `overflow-hidden`(`apps/web/src/components/ui/card.tsx:15`)이 이를 전부 클리핑해 사용자에게 픽셀상 전혀 보이지 않음 — z-index 문제가 아니라 클리핑 계층 문제라 WP-D의 z-50 조치로는 해결되지 않았음. **수정 브랜치 `fix/toast-clipping`에서 수정 작업 진행 중 — 수정 PR 출하 예정.** 자율도(`computeAutonomy`) 갱신은 이번 최종검증 스코프 밖(프로젝트 허브 Phase 2, 2026-06-30, DEV_REFERENCE §3.J에서 기 검증)으로 그대로 유지. 증거: `.agents/results/2026-07-07-final-verification.md` §⑥ + 스크린샷(`final-verify-screens/06-*.png`).
