...
### P0-2. 비파괴적 · 멱등 복원 시드
- 현재 `import-finance-csv.ts`는 **전체 deleteMany 후 재생성**이라 자금흐름(은행 실거래)까지 날림 → 위험.
- **할 일**:
  - 임포터를 **테이블별 분리** + **upsert 기반**으로 전환 (cashflow는 절대 일괄삭제 금지).
  - `pnpm cfo:restore` 단일 명령으로 **언제든 안전 복원** (기존 은행 자금흐름 보존).
- **수용 기준**: 임포터 재실행 시 자금흐름 179건 유지 + 마스터 데이터 정합.
