...
- [01 WP-D, 2026-07-07] AI 커맨드 토스트가 활동로그 카드에 가려 실사용자에게 거의 안 보임(z-index/stacking, `ai-command-bar.tsx` — WP-D 이전부터 존재하던 기존 결함, 이번 라운드에서 텍스트/네트워크 계약만 바뀜). SSE 스트리밍 UI 자체의 고도화(타이핑 효과·중간 진행상황 노출 등)도 미착수. 근거: `.agents/results/2026-07-07-wp-d-live.md` — 담당 문서 미정.
