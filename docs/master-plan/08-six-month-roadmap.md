...
- [ ] **[F4] 미수금 독촉 초안 (AR nudge)**: 만기 경과 미수금(14건 실존) → 독촉 메일 **초안** 생성 → 승인 큐. **실측 — 발송 능력 부재(Graph 스코프 Mail.Read뿐, 발송 코드 0). 첫 외부 발송 능력이므로 ADR-003(발신 게이트: Mail.Send 스코프·위험작업 승인·결정 스파인 기록·수신자 화이트리스트) 선행 필수.** M4는 초안+승인 큐까지, 실발송은 ADR 승인 후.
...
| F4 | AR 독촉 초안 | 미수금 14건 추적 수동 | M | **M4** | **발송 능력 부재 → ADR-003 선행** |
