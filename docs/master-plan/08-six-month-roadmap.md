...
- [~] **Task 0 — ADR-002**: 작성 완료(2026-07-10, `docs/convergence/ADR-002-api-surface.md` — web=BFF 채택, tRPC 도입 폐기, Phase 7=인덱스+FK만). 실측 인풋: REST 97 route, tRPC 클라이언트 저장소 전체 0, web tRPC=hello 스텁. **사용자 승인(→Accepted)만 잔여.**
- [x] **리팩토링 Phase 2·3·4** — 2.5 프로토콜 실측 결과 **로드맵 이 항목 자체가 낡아 있었음**: 셋 다 이미 머지 완료(Phase 2=PR #85, Phase 3=11라우트 커밋군, Phase 4=b955a19+627a072). 2026-07-10에 잔여 갭 2건만 추가 완결 — 3-I dashboard/[role] prisma 12건 추출(응답 9종 바이트 동일 검증) + 죽은 헬퍼(스테일 berlo 라벨) 제거. 상태 정본은 03 문서 기준선 표(2026-07-10 갱신).
- [x] 게이트: 02 검증서(G1~G4) + golden 무변화 + 시나리오 1 라이브 재연 통과 — 재연이 task 전환 createdEntityId 미설정 실버그를 검출, 즉시 수정+회귀테스트+백필(증거 `.agents/results/2026-07-10-w2-w4-gate.md`).

### Task M1-7 [F2]: 아침 브리핑 푸시 (stretch — W1 6태스크와 03 게이트가 우선, 여유 시 W2에)
**가치 근거**: 현재 큐는 사용자가 들어가서 봐야 한다(pull). 매일 08:00 푸시(push)로 리뷰 습관을 시스템이 만든다 — L3 자율화의 병목인 **자율도 표본 축적**이 빨라지는 지렛대.
**Files**: 신규 `packages/business/src/reporting/daily-brief.ts` (+test) · 신규 `scripts/daily-brief.sh` · 기존 `generateDailyReport`(재사용) · 채널 어댑터 1개(텔레그램 봇 권장 — 토큰은 .env, 커밋 금지)
**Interfaces**: `buildDailyBrief(): Promise<{ decisionsTop5, deadlines, inflowSummary, runwayDelta }>` — 큐 상위5(confidence 정렬)+마감 임박+어제 인입 요약+런웨이 변화
- [ ] Step 1 (TDD): buildDailyBrief 단위 테스트 — 빈 큐/만기 없음/정상 3케이스
- [ ] Step 2: 구현 — 기존 daily-report·approvals 쿼리 재사용, 새 쿼리 최소화
- [ ] Step 3: 채널 발송(사용자에게 텔레그램 vs 메일 1택 확인 — **외부 발송이므로 사용자 승인 후 활성화**)
- [ ] Step 4: M1-5 스케줄에 08:00 동승
**Acceptance**: 3일 연속 08:00 수신 + 내용이 당일 큐/DB와 일치(스팟 체크).

**M1 종료 기준**: KPI 기준선 확정, 큐 리드타임 첫 측정치, 재무 연결률 ≥60%(또는 상한 명시), 03 문서 체크박스 ≥80%, 데일리 배치 3일 연속 가동.

**M1 종료 판정 (2026-07-10)**: ① KPI 기준선 확정 ✅(`kpi-20260709.txt` — 정밀도 51.0%, 자동승인 0%[구조적 정상]) ② 큐 리드타임 첫 측정 ✅(평균 3.22일/중앙값 2.63일, updated_at 근사 한계 주석 포함) ③ 재무 연결률 ✅(12.7% + 실데이터 상한 명시 — projectId 보유 행이 38/229뿐이라 60%는 구조적으로 불가, M2 데이터 위생으로 이월) ④ 03 문서 체크박스 ✅(≈88%, 잔여=ADR-002 사용자 승인·지식그래프 재학습[M4 선행조건으로 이월]) ⑤ 데일리 배치 3일 연속 ⏳(launchd 등록 2026-07-10 — 최초 관측 가능일 2026-07-13, **유일한 시간 종속 잔여**). 4/5 충족, ⑤는 달력 대기.
...
- [ ] **[F4] 미수금 독촉 초안 (AR nudge)**: 만기 경과 미수금(실측 2026-07-08: 미수 7·부분 2 — 집행 시 재실측) → 독촉 메일 **초안** 생성 → 승인 큐. **실측 — 발송 구현 0건(실사용 동기화는 Mail.Read만 요청; 단 `packages/auth/src/token-manager.ts`에 휴면 Mail.Send 스코프 상수가 이미 존재 — 발신 게이트가 이를 관장해야 함). 첫 외부 발송 능력이므로 ADR-003(발신 게이트: Mail.Send 활성화·위험작업 승인·결정 스파인 기록·수신자 화이트리스트) 선행 필수.** M4는 초안+승인 큐까지, 실발송은 ADR 승인 후.
- [ ] **[F6] 파트너 스코어카드**: 파트너별 딜 기여·응답속도·마진 추이 뷰. **선행: M2 파트너 재구축(3→49행)** — 미완이면 M5로 밀 것.
- [ ] **[F9] What-if 런웨이 시뮬레이터** (filler — 순수 프론트): 기존 computePnl·런웨이 게이지 재사용, "딜 지연/매입 당김" 슬라이더 → 런웨이 변화. 백엔드 변경 0.
...
- [ ] **[F7] 회의록→액션 파이프라인**: 회의 녹취(로컬 STT — 경로 실증 선행) → 요약 → 액션아이템 → WorkTask 자동 생성 제안(승인 큐). **실측 — meeting_notes 0행이므로 캘린더 동기화(기존 P7 코드) 재가동이 1스텝.**
- [ ] **[F8] 음성 브리핑·승인**: F2 브리핑 콘텐츠를 로컬 TTS로 낭독 + "N번 승인" 음성 명령(위험작업 게이트 뒤, F7의 STT 재사용). 이동 시간을 리뷰 시간으로 — 자율도 표본 축적 가속.
...
- [ ] **F-트랙 회고**: F1~F10 중 미출시분의 2027 이월/폐기 결정 (사용률 데이터 기반 — 안 쓰는 기능은 정직하게 폐기).
...
## 8.5 F-트랙 — 기능 확장 10선 (2026-07-08 아이디에이션, 사용자 채택)

**배치 원칙**: F-트랙은 코어 트랙(03~07 실행)의 **2순위**다. 각 월의 코어 게이트를 먼저 충족하고 잔여 용량에 F를 당긴다 — 코어가 늦어지면 F가 밀리는 것이지 그 역이 아니다. 모든 F 항목은 §2.5 월초 상세화 프로토콜을 따르며, 아래 표의 "실측 전제"가 깨져 있으면 상세화 단계에서 로드맵을 수정한다.

| # | 기능 | 가치 근거 (실측) | 크기 | 배치 | 의존/전제 (2026-07-08 실측) |
|---|---|---|---|---|---|
| F1 | 리뉴얼 레이더 | 딜의 태반이 리뉴얼(GS건설/UNID/롯데…) — 지금은 메일이 와야 앎 | L | **M2** (A: 원장 부트스트랩 → B: 레이더) | 스키마 완비·**데이터 0행** → 부트스트랩 선행 |
| F2 | 아침 브리핑 푸시 | 큐 리뷰를 pull→push로 — 자율도 표본 축적 가속 | S | **M1 stretch** (Task M1-7) | daily-report 실존·채널 1택 사용자 확인 |
| F3 | 견적서 자동 초안 | 이중게이트 통과 12건 전원이 견적/사이징 요청 | M | **M3** | 승격 루프 실존·quotes 0행(문서 축으로) |
| F4 | AR 독촉 초안 | 미수금 추적 수동 (실측 2026-07-08: 미수 7·부분 2 — 집행 시 재실측) | M | **M4** | **발송 능력 부재 → ADR-003 선행** |
| F5 | 딜 전투 카드 | 데이터 연결 완료(v1) — 뷰만 부재 | S-M | **M2** | 프로젝트 허브 조인 재사용 |
| F6 | 파트너 스코어카드 | FP.client=파트너 확인 — 리드 배분 근거 | S-M | **M4** | M2 파트너 재구축(3→49) 선행 |
| F7 | 회의록→액션 | 메일 밖 업무의 절반은 회의 | M | **M5** | meeting_notes 0행 → 캘린더 동기화 재가동부터 |
| F8 | 음성 브리핑·승인 | 이동 시간→리뷰 시간 | M | **M5** | F2+F7(STT/TTS) 재사용, 위험작업 게이트 |
| F9 | What-if 런웨이 | CFO 결정 지원 — 백엔드 0 | S | **M4 filler** | computePnl·게이지 재사용 |
| F10 | 주간 AI 회고 | M3 자동화의 신뢰 장치 | S | **M3** (autopilot 동시) | KPI 리포트(M1-6) 확장 |

**우선 가치 서열**(용량 충돌 시 이 순서로 생존): F1 > F2 > F10 > F5 > F3 > F4 > F6 > F9 > F7 > F8.
**M2 과밀 주의**: M2는 코어만으로 만재(스파인+파트너 재구축+복원드릴). F1 Phase B는 **Phase A 완료가 게이트**이며, 코어 지연 시 서열과 무관하게 **Phase B를 M3로 우선 이월**한다(월 게이트가 서열보다 우선 — 큰 항목을 남기고 작은 것을 자르는 역설 방지).
**KPI 연결**: F2·F10은 §2 KPI의 측정·전달 수단을 겸한다(별도 지표 아님). F1은 "리뉴얼 기회 선행 생성률"(만기 D-60 전 기회화 비율, M2에 지표 추가)로 측정.
...
- **2026-07-08 (5차, F-트랙 표적 검증 반영)**: momus APPROVE — 정정 3건: 미수금 실측치(14→미수7·부분2), 휴면 Mail.Send 스코프 상수 존재를 ADR-003 관장 범위로 명기, M2 과밀 밸브(F1 Phase B는 Phase A 게이트+M3 우선 이월).
- **2026-07-08 (4차, F-트랙 편입)**: 기능 확장 10선(F1~F10)을 §8.5 총괄표+월별 체크박스로 편입 — 실측 선행(CustomerAsset·quotes·meeting_notes 0행, 발송 능력 부재)을 각 항목 전제로 명기, 코어 2순위 원칙과 생존 서열 명시. F2는 M1-7로 태스크 표준 상세화.
