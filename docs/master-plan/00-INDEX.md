...
### 2026-07-10 갱신
**M1 W1~W4 사실상 완결.** W1 6태스크(캘리브레이션 운영 마감 §9 포함) + W2~W4 완료. 핵심 발견: 로드맵 08 §3의 "Phase 2 잔여→3→4" 항목은 2026-07-04 기준선이라 낡아 있었고, **실측(2.5 프로토콜) 결과 셋 다 이미 머지돼 있었음** — 실제 신규 작업은 ADR-002 작성(web=BFF 채택, 승인 대기) + 잔여 갭 2건(dashboard/[role] prisma 12건 추출, 죽은 헬퍼 제거)이었다. 02 검증 게이트(G1~G4+golden+시나리오1) 통과 — 시나리오 1 재연이 task 전환 createdEntityId 미설정 실버그를 검출해 즉시 수정·회귀테스트·백필. M1 종료 기준 4/5 충족(잔여 1건은 데일리 배치 3일 연속 가동 — 2026-07-13 이후 관측). 증거: `.agents/results/2026-07-10-w2-w4-gate.md`. 상태 정본: 03 문서 기준선 표.
...
**알려진 문서 간 상충 2건 — 해소됨** (2026-07-10, `docs/convergence/ADR-002-api-surface.md`, 사용자 승인 대기):
- Phase 6 방향: **마스터플랜 채택** — web=BFF, 미사용 tRPC 표면 제거(실측: 저장소 전체 tRPC 클라이언트 0). phase-6 실행 문서에 폐기 배너 추가됨.
- Phase 7 내용: **마스터플랜 채택** — 인덱스·FK 승격만. 단 phase-6 문서의 신규 컬럼(segment/riskScore)은 2026-07-03 기추가분이라 수용, 추가 예측 컬럼만 07 전 금지(ADR-002 D4).
