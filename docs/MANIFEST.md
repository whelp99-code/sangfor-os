# Manifest

## Canonical requirements and acceptance

- [Canonical ID registry](01_SPEC/Requirement_ID_Registry.md) — precedence, aliases, 28-requirement/71-acceptance scope, and C/W exclusions.
- [Requirements — MoSCoW](01_SPEC/Requirements_MoSCoW.md) — canonical requirement source.
- [Acceptance Criteria & Test Plan](08_IMPLEMENTATION/Acceptance_Criteria_Test_Plan.md) — canonical acceptance source.
- [Acceptance manifest](12_VERIFICATION/acceptance-manifest.json) — 99-row owner/execution/closure/test manifest.
- [Acceptance evidence schema](12_VERIFICATION/acceptance-evidence.schema.json) — evidence receipt contract.

## Package file index

- `00_EXECUTIVE/Executive_Brief.md`
- `01_SPEC/Requirement_ID_Registry.md`
- `01_SPEC/Requirements_MoSCoW.md`
- `01_SPEC/SPEC-3-Agentic-Company-OS-SANGFOR-Partner-OS-Final.md`
- `01_SPEC/SPEC-3.1-Code-Skeleton-Governance-Patch.md`
- `02_ARCHITECTURE/API_Contract.md`
- `02_ARCHITECTURE/Core_Architecture.md`
- `02_ARCHITECTURE/Database_ERD_Schema.md`
- `02_ARCHITECTURE/Workflow_Approval_Model.md`
- `03_BUSINESS/Partner_Workflows.md`
- `03_BUSINESS/Product_SKU_License_Quote_Model.md`
- `03_BUSINESS/SANGFOR_Partner_Business_Fit.md`
- `04_SECURITY/Audit_And_Compliance.md`
- `04_SECURITY/Auth_RBAC_ABAC_RLS.md`
- `04_SECURITY/Security_Threat_Model.md`
- `05_DATA_AI/AI_Quality_Governance.md`
- `05_DATA_AI/Data_Governance.md`
- `06_UX/Role_Based_UX_Dashboards.md`
- `07_OPERATIONS/Deployment_Operations_Guide.md`
- `07_OPERATIONS/Operator_Runbook.md`
- `08_IMPLEMENTATION/Acceptance_Criteria_Test_Plan.md`
- `08_IMPLEMENTATION/Contractor_Work_Packages.md`
- `08_IMPLEMENTATION/Implementation_Plan_Milestones.md`
- `09_ROI/Cost_ROI_Model.md`
- `10_CODE_SKELETON/README.md`
- `10_CODE_SKELETON/backend/app/ai_quality.py`
- `10_CODE_SKELETON/backend/app/approvals.py`
- `10_CODE_SKELETON/backend/app/assets.py`
- `10_CODE_SKELETON/backend/app/auth.py`
- `10_CODE_SKELETON/backend/app/certifications.py`
- `10_CODE_SKELETON/backend/app/data_exports.py`
- `10_CODE_SKELETON/backend/app/main.py`
- `10_CODE_SKELETON/backend/app/poc.py`
- `10_CODE_SKELETON/backend/app/policies.py`
- `10_CODE_SKELETON/backend/app/quotes.py`
- `10_CODE_SKELETON/backend/app/renewals.py`
- `10_CODE_SKELETON/backend/app/vendor.py`
- `10_CODE_SKELETON/db/rls.sql`
- `10_CODE_SKELETON/db/schema.sql`
- `10_CODE_SKELETON/frontend/app/page.tsx`
- `10_CODE_SKELETON/seed/sangfor_industry_pack_v3.json`
- `11_REFERENCES/References.md`
- `12_VERIFICATION/Final_Document_Coverage_Check_Report_V3_1.md`
- `12_VERIFICATION/acceptance-manifest.json`
- `12_VERIFICATION/acceptance-evidence.schema.json`
- `12_VERIFICATION/test-alias-map.json`
- `MANIFEST.md`
- `README_전체_진행_가이드.md`


---

# V3.2 Added Files

```text
01_SPEC/SPEC-3.2-Hermes-Color-Agent-Organization-Patch.md
02_ARCHITECTURE/Hermes_Color_Agent_Architecture.md
02_ARCHITECTURE/Color_Agent_Kanban_Handoff_Model.md
03_BUSINESS/Color_Agent_Business_Impact.md
06_UX/Color_Agent_UX_Dashboard.md
07_OPERATIONS/Color_Agent_Operations_Runbook.md
13_COLOR_AGENT_ORG/README.md
13_COLOR_AGENT_ORG/Color_Role_Definitions.md
13_COLOR_AGENT_ORG/Kanban_Handoff_Template.md
13_COLOR_AGENT_ORG/Color_Agent_Routing_Rules.md
13_COLOR_AGENT_ORG/SANGFOR_Color_Mapping.md
13_COLOR_AGENT_ORG/Color_Agent_Adoption_Guide.md
10_CODE_SKELETON/db/color_agents.sql
10_CODE_SKELETON/seed/color_agent_registry_v3_2.json
10_CODE_SKELETON/backend/app/color_agents.py
10_CODE_SKELETON/backend/app/handoffs.py
10_CODE_SKELETON/backend/app/color_review_gates.py
10_CODE_SKELETON/frontend/app/color-agents/page.tsx
12_VERIFICATION/Final_Document_Coverage_Check_Report_V3_2.md
```
