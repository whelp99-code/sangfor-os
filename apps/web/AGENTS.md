...
- Styling: Tailwind v4 + design tokens in `app/globals.css`, shared primitives from `@sangfor/ui`, local shadcn in `components/ui/`. URL state via `nuqs`. Follow [DESIGN.md](../../DESIGN.md).
- Colocated `*.test.ts` (Vitest, `@`→`src`); E2E specs in `tests/e2e/playwright` (`baseURL :3101`).

## Dependencies
- Depends on: `@sangfor/business`, `@sangfor/db`, `@sangfor/agent`, `@sangfor/infra`, `@sangfor/mail-intelligence`, `@sangfor/ui`, `@sangfor/shared`.
