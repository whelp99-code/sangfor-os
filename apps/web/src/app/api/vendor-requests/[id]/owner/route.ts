...
export async function PATCH(
  ...
  try {
    ...
    const { ownerAssignmentId, expectedOwnershipRevision } = body;

    if (!ownerAssignmentId || typeof expectedOwnershipRevision !== "number") {
      return NextResponse.json(
        { error: "ownerAssignmentId and expectedOwnershipRevision are required" },
        { status: 400 },
      );
    }
    ...
    const result = await reassignVendorRequestOwner({
      authContext,
      requestId,
      ownerAssignmentId,
