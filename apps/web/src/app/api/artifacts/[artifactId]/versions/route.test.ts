...
function env() { return { USER_JWT_ACTIVE_KID: "artifact-route-key", USER_JWT_ROTATION_OWNER: "security-auth", USER_JWT_ISSUER: "sangfor-os", USER_JWT_AUDIENCE: "sangfor-os-runtime", USER_JWT_TTL_SECONDS: "28800", USER_JWT_CLOCK_SKEW_SECONDS: "30", USER_JWT_KEYRING_JSON: JSON.stringify({ version: "sangfor.user-jwt-keyring/v1", keys: [{ kid: "artifact-route-key", state: "active", secretBase64Url: Buffer.alloc(32, 1).toString("base64url"), activatedAt: new Date(Date.now() - 60_000).toISOString().replace(/\.\d{3}Z$/, "Z"), demotedAt: null, verifyUntil: null, retiredAt: null }] }) }; }
