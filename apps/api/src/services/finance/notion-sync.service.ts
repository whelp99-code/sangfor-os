...
export class NotionSyncService {
  ...
  async triggerCsvImport() {
    const exportDir = process.env.NOTION_EXPORT_DIR;
    if (!exportDir) return { ok: false, error: 'Set NOTION_EXPORT_DIR in .env to the Notion CSV export folder' };
    const script = path.join(process.cwd(), 'scripts/import-csv-to-prisma.ts');
    try {
      const { stdout, stderr } = await execFileAsync('pnpm', ['exec', 'tsx', script], {
