...
function promisify<T = any>(fn: any, ...boundArgs: any[]) {
  return (...args: any[]) =>
    new Promise<T>((resolve, reject) => {
      ...
      try { fn(...boundArgs, ...args, success, error); } catch (e) { reject(e); }
    });
}
...
export class PopbillService {
  ...
  private getCorpNum(): string {
    ...
    if (!num) throw new Error('POPBILL_CORP_NUM 환경변수가 설정되지 않았습니다.');
  ...
  async checkStatus() {
    ...
    try {
      ...
      return { enabled: true, testMode: process.env.POPBILL_IS_TEST === 'true', chargeInfo };
    } catch (e: any) {
      console.error(`팝빌 상태 확인 실패: ${e?.message ?? e}`);
  ...
  async issue(input: IssueTaxInvoiceInput) {
    ...
    if (input.items.length === 0) {
      throw new Error('품목이 최소 1개 이상 필요합니다.');
    }

    const mgtKey = input.mgtKey ?? `INV-${Date.now()}-${Math.random().toString(36).slice(2, 8).toUpperCase()}`;
    const record = await prisma.taxInvoice.create({
      data: {
        invoiceId: input.invoiceId, projectId: input.projectId,
        direction: input.direction, status: this.isEnabled() ? 'requested' : 'draft',
        supplierCorpNum: input.supplierCorpNum, supplierName: input.supplierName,
        buyerCorpNum: input.buyerCorpNum, buyerName: input.buyerName,
        supplyAmount: input.supplyAmount, vatAmount: input.vatAmount, totalAmount: input.totalAmount,
        issueDate: input.issueDate, memo: input.memo,
        rawResponse: JSON.stringify({ mgtKey, source: 'cfo-aios' }),
      },
    });

    if (!this.isEnabled()) {
      console.log(`[MOCK] 세금계산서 발행: ${record.id} mgtKey=${mgtKey}`);
    ...
    try {
      ...
      const taxInvoice = {
        writeDate: issueDate, chargeDirection: '정과금', purposeType: '영수',
        supplyCostTotal: String(input.supplyAmount), taxTotal: String(input.vatAmount),
        totalAmount: String(input.totalAmount), remark1: input.memo ?? '',
        invoicerCorpNum: input.supplierCorpNum.replace(/-/g, ''), invoicerCorpName: input.supplierName,
        invoicerCEOName: input.supplierCEOName ?? '', invoicerAddr: input.supplierAddr ?? '',
        invoicerBizType: input.supplierBizType ?? '', invoicerBizClass: input.supplierBizClass ?? '',
        invoiceeCorpNum: input.buyerCorpNum.replace(/-/g, ''), invoiceeCorpName: input.buyerName,
        invoiceeCEOName: input.buyerCEOName ?? '', invoiceeAddr: input.buyerAddr ?? '',
        invoiceeBizType: input.buyerBizType ?? '', invoiceeBizClass: input.buyerBizClass ?? '',
        invoiceeEmail1: input.buyerEmail ?? '',
        detailList: input.items.map((it, i) => ({
          serialNum: String(i + 1), itemName: it.name, qty: String(it.qty),
          unitCost: String(it.unitPrice), amount: String(it.amount), tax: '과세',
        })),
      };
      ...
      const updated = await prisma.taxInvoice.update({
        where: { id: record.id },
        data: { status: 'transmitted', rawResponse: JSON.stringify({ mgtKey, sdkResult: 'success' }) },
      });
      return { ok: true, mock: false, taxInvoice: updated, mgtKey };
    } catch (e: any) {
      console.error(`팝빌 발행 실패: ${e?.message ?? e}`);
      await prisma.taxInvoice.update({
        where: { id: record.id },
        data: { status: 'failed', rawResponse: JSON.stringify({ mgtKey, error: e?.message ?? String(e) }) },
      });
  ...
  async collectPurchaseTaxInvoices(year: number, month: number) {
    ...
    try {
      ...
      console.error(`홈택스 조회 실패: ${e?.message ?? e}`);
  ...
  async checkBizInfo(corpNum: string) {
    if (!this.isEnabled()) {
      return { ok: true, mock: true, corpNum, status: '정상', message: '팝빌 미설정, mock 정상 응답' };
    }
    try {
  ...
  async listHistory(opts: { direction?: string; status?: string; limit?: number } = {}) {
    return prisma.taxInvoice.findMany({
      where: { direction: opts.direction, status: opts.status },
