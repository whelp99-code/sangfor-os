...
> Express 4 + tRPC 11 backend on port 3200. NOT the primary product API (that is `apps/web` route handlers). This service authoritatively owns the **CFO/finance domain** plus Outlook webhooks, Prometheus metrics, SSE events, and the MCP-bridge proxy.
...
- **Guard every finance route.** REST `/api/cfo/*` (beyond public health) requires API key + `financeAccessGuard` (system_admin/finance_manager/ceo). Middleware order: api-key → auth → rbac → finance-access → rate-limiter → error-handler.
...
## Working Here
- REST routes in `src/routes/` (`cfo.ts`, `events.ts`); tRPC in `src/routers/` (`cfo/` has 17 sub-routers). `createApp()` in `src/index.ts` wires everything.
